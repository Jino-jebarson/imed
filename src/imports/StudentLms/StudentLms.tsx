...
import { Bell, BookOpen, CalendarDays, Camera, CheckCircle2, ChevronLeft, ChevronRight, Clock, Download, ExternalLink, Eye, EyeOff, FileText, IndianRupee, LayoutDashboard, Lightbulb, LogOut, MessageSquareText, NotebookPen, Stethoscope, UserRound, X } from "lucide-react";
import { Cell, Pie, PieChart, ResponsiveContainer } from "recharts";
import { Toaster, toast } from "sonner";
import imedLogo from "../HomePage/imedLogo.png";
      ...
      <>
        ...
        <main className="slms-page slms-login-page">
          ...
          <form className="slms-login-card" onSubmit={login}>
            <div className="slms-brand"><span><img src={imedLogo} alt="iMED Academy" /></span><div><b>iMED Academy</b><small>Student LMS</small></div></div>
  ...
  return (
    <>
      ...
      <aside className={`slms-sidebar ${sidebarOpen ? "open" : ""}`}>
        <div className="slms-brand"><span><img src={imedLogo} alt="iMED Academy" /></span><div><b>iMED Academy</b></div><button type="button" className="slms-sidebar-close" aria-label="Close menu" onClick={() => setSidebarOpen(false)}><X size={17} /></button></div>
        <nav className="slms-nav-groups">
          ...
          <div className="slms-nav-group"><p>Classroom</p><button className={tab === "timetable" ? "active" : ""} onClick={() => changeTab("timetable")}><CalendarDays size={17} /> Timetable</button><button className={tab === "attendance" ? "active" : ""} onClick={() => changeTab("attendance")}><CheckCircle2 size={17} /> Attendance</button><button className={tab === "progress" ? "active" : ""} onClick={() => changeTab("progress")}><BookOpen size={17} /> Progress</button><button className={tab === "notes" ? "active" : ""} onClick={() => changeTab("notes")}><NotebookPen size={17} /> Notes{unreadNotesCount > 0 && <em className="slms-nav-badge">{unreadNotesCount > 9 ? "9+" : unreadNotesCount}</em>}</button></div>
